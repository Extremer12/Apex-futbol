...
export const StadiumScreen: React.FC<StadiumScreenProps> = ({ gameState, dispatch }) => {
    ...
    const handleUpgradeFacilities = () => {
        ...
        if (stadium.facilityLevel >= 5) {
            showToast("Facilities are already at maximum level.", 'info');
            return;
        }
        if (finances.balance < cost) {
            showToast(`You need ${formatCurrencyShort(cost)} for this upgrade.`, 'error');
            return;
        }
        ...
        showToast("Facilities upgraded successfully!", 'success');
    };

    const handleStartExpansion = () => {
        const cost = stadium.expansionCost || (stadium.capacity * 1000);
        if (finances.balance < cost) {
            showToast(`You need ${formatCurrencyShort(cost)} for the expansion.`, 'error');
            return;
        }
        ...
        dispatch({ type: 'UPDATE_FINANCES', payload: { ...finances, balance: finances.balance - cost, balanceHistory: [...finances.balanceHistory, finances.balance - cost] } });
        showToast(`Expansion complete. New capacity: ${newCapacity.toLocaleString()} seats.`, 'success');
    ...
    return (
        <div className="p-4 md:p-6 space-y-6 pb-24 animate-fade-in">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/10 pb-4">
                <div>
                    <h2 className="text-[10px] font-black text-gold-gradient tracking-[0.3em] uppercase mb-1">Infrastructure</h2>
                    <h1 className="text-3xl font-black text-white uppercase italic tracking-tighter">Stadium</h1>
                </div>
            </div>
                ...
                <div className="md:col-span-2 apex-card relative overflow-hidden group">
                    ...
                    <div className="relative z-10 p-6 space-y-8">
                        <div>
                            ...
                            <p className="text-[var(--apex-gold)] font-black uppercase tracking-[0.2em] text-[10px]">Official Venue</p>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-black/40 p-4 rounded-xl border border-white/5">
                                <div className="text-white/40 text-[9px] font-black uppercase tracking-widest mb-1">Total Capacity</div>
                                <div className="text-3xl font-black text-white">{stadium.capacity.toLocaleString()}</div>
                            </div>
                            <div className="bg-black/40 p-4 rounded-xl border border-white/5">
                                <div className="text-white/40 text-[9px] font-black uppercase tracking-widest mb-1">Ticket Price</div>
                        ...
                        <div className="pt-2">
                            <div className="flex justify-between items-center mb-2">
                                <span className="text-[9px] font-black text-white/50 uppercase tracking-widest">Facility Level</span>
                                <span className="text-[9px] font-black text-[var(--apex-gold)] uppercase tracking-widest">Level {stadium.facilityLevel}/5</span>
                            </div>
                ...
                <div className="apex-card p-6 flex flex-col">
                    ...
                    <div className="space-y-4 flex-1">
                        <div className="flex justify-between items-center pb-3 border-b border-white/5">
                            <span className="text-white/50 text-[10px] font-bold uppercase tracking-widest">Weekly Upkeep</span>
                            <span className="text-[var(--apex-red)] font-black">-{formatCurrencyShort(stadium.maintenanceCost)}</span>
                        </div>
                        <div className="flex justify-between items-center pb-3 border-b border-white/5">
                            <span className="text-white/50 text-[10px] font-bold uppercase tracking-widest">VIP Revenue (Bonus)</span>
                            <span className="text-[var(--apex-green)] font-black">+{Math.floor((stadium.facilityLevel - 1) * 12.5)}%</span>
                        </div>
                        <div className="pt-4 mt-auto">
                            <div className="text-white/40 text-[9px] font-black uppercase tracking-[0.2em] mb-1">Estimated Full-House Revenue</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                    <div className="flex items-start gap-4 relative z-10">
                        ...
                        <div className="flex-1">
                            <div className="font-black text-white text-lg uppercase tracking-tight mb-1">Expand Stands</div>
                            <p className="text-white/50 text-[10px] font-bold uppercase tracking-widest leading-relaxed">Increase capacity to welcome more fans and generate higher matchday revenue.</p>
                            <div className="mt-4 flex justify-between items-center">
                                <span className="text-[9px] text-white/30 uppercase font-black tracking-widest">Cost starts from</span>
                ...
                >
                    <div className="flex items-start gap-4 relative z-10">
                        ...
                        <div className="flex-1">
                            <div className="font-black text-white text-lg uppercase tracking-tight mb-1">Upgrade Facilities</div>
                            <p className="text-white/50 text-[10px] font-bold uppercase tracking-widest leading-relaxed">Enhance VIP areas and general services to increase average spend per ticket.</p>
                            <div className="mt-4 flex justify-between items-center">
                                <span className="text-[9px] text-white/30 uppercase font-black tracking-widest">Upgrade Cost</span>
            ...
            {isExpanding && (
                <Modal title="Confirm Expansion" onClose={() => setIsExpanding(false)}>
                    <div className="space-y-6">
                        <p className="text-white/70 text-sm leading-relaxed">Are you sure you want to expand the stadium? This project will increase total capacity by 25%.</p>
                        
                        <div className="bg-black/50 p-5 rounded-xl border border-white/5 space-y-4">
                            <div className="flex justify-between items-center pb-3 border-b border-white/5">
                                <span className="text-white/50 font-black uppercase text-[10px] tracking-widest">Project Cost</span>
                                <span className="text-white font-black">{formatCurrencyShort(stadium.capacity * 1000)}</span>
                            </div>
                            <div className="flex justify-between items-center">
                                <span className="text-white/50 font-black uppercase text-[10px] tracking-widest">New Capacity</span>
                                <span className="text-[var(--apex-gold)] font-black text-lg">{Math.floor(stadium.capacity * 1.25).toLocaleString()}</span>
                            </div>
                        </div>
                        
                        <div className="flex gap-3 pt-2">
                            <button onClick={() => setIsExpanding(false)} className="flex-1 py-3 bg-white/5 text-white/50 font-black rounded-lg hover:bg-white/10 transition-all text-xs uppercase tracking-widest border border-white/10">CANCEL</button>
                            <button onClick={handleStartExpansion} className="flex-1 py-3 apex-btn-gold text-xs !w-auto">START PROJECT</button>
                        </div>
                    </div>
                </Modal>
            )}
